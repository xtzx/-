...
function LinkList() {
    ...
    var get_node = function (index) {
        ...
        while (node_index-- > 0) { // while循环只循环index次，头节点为index为0
    ...
    this.insert = function (index, data) {
        if (index == length) { // index == length,说明是在尾节点的后面新增,直接调用append方法即可
            return this.append(data);
        } else if (index > length || index < 0) {
            return false; // index范围错误
        } else {
            ...
            if (index == 0) {
                ...
                new_node.next = pre_node.next; // 连接下一个
                pre_node.next = new_node; // 连接上一个
            }
            length += 1;
            return true;
        }
     ...
     * 方法虽然简单， 但涉及到了链表最重要的操作-- - 遍历链表
     * 们平时遍历数组时， 直接使用for循环， 通过索引的变化来获得数组的不同位置的值
     * 但对于链表， 我们只能通过next指针找到一个节点的下一个节点。
     */
    this.print = function () {
        var curr_node = head;
        var str_link = ""
